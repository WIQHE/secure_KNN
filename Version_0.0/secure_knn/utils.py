...
def generate_m_temp(eta: int, q_max: float, max_norm: float) -> np.ndarray:
    ...
    # np.random.randint requires Python ints; max_norm/q_max may arrive as floats.
    max_norm_int = int(np.ceil(max_norm))
    q_max_int = int(np.ceil(q_max))
    while True:
        matrix = np.random.rand(eta, eta)
        np.fill_diagonal(
            matrix,
            np.random.randint(max_norm_int, max_norm_int + 100, size=eta),
        )
        mask = np.eye(eta, dtype=bool)
        matrix[~mask] = np.random.randint(
            q_max_int, q_max_int + 100, size=eta * eta - eta
        )
...
def encrypt_original_data_user_cloud(
    ...
    for i in range(n):
        ...
        # Per SecureNN-ICISS-2023.pdf §2 (Data Encryption):
        #   p̂i = (s1 - 2*pi1, ..., sd - 2*pid, s_{d+1} + ||pi||^2, τ, vi)
        # The (d+1)-th coordinate is ||pi||^2 = sum(pi_j^2).
        norm_squared = float(np.sum(pi[:d] ** 2))
